...
def DeepNetwork1D(input_size=128, output_size=128, hidden_size=512,
                 kernels  =[1, 3, 3, 3, 3, 3, 3, 3, 3, 3, 3, 3, 3, 3, 3],
                 dilations=[1, 1, 1, 1, 2, 2, 2, 2, 2, 2, 4, 4, 4, 4, 4],
...
class IDreveal(torch.nn.Module):
    def __init__(self, time, weights_file='./model.th'):
        super(IDreveal, self).__init__()  
        self.time = time
        self.htime = (self.time-50)//2
        self.network = DeepNetwork1D(256, 128, 512,
                 kernels  =[1, 3, 3, 3, 3, 3, 3, 3, 3, 3, 3],
                 dilations=[1, 1, 1, 1, 2, 2, 2, 4, 4, 4, 4],
                 last_act=False, flag_residue_first=False, nangroup=True)
        

    def forward(self, data, stride=1):
        size = data.size()
        out = torch.empty((size[0], size[1]-70, 128), device=data.device)
        for i in range(len(data)):
            clips = data[i].unfold(0, self.time, stride)
            out[i] = self.network(clips)[:,:,self.htime]
        return out
